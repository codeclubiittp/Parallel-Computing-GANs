...
float computeMSELoss(const float* h_predictions, const float* h_targets, int size) {
    ...
    squaredDifferenceKernel << <blocksPerGrid, threadsPerBlock >> > (
        d_predictions, d_targets, d_differences, size);
